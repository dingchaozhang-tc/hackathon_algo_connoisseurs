...
def get_kernel(kernel_size, blur=1 / 20, halo=.001):
    ...
    kernel = np.exp(-r / kernel_size / blur) + \
        (1 - r / r[center, 0]).clip(0) * halo
    return kernel
...
def add_lines(image_array, xys, width=1, weights=None):
    ...
    # convolve image
    new_image_array = scipy.ndimage.filters.convolve(
        image_array, get_kernel(width * 4))
...
def get_min_max(car_data):
    min_lat = car_data["Start Station Latitude"].min()
    max_lat = car_data["Start Station Latitude"].max()
    max_lon = car_data["Start Station Longitude"].max()
    min_lon = car_data["Start Station Longitude"].min()
...
def plot_station_counts(trip_counts, center_loc=[32.8377, -96.784], zoom_start=13):
    # generate a new map
    folium_map = folium.Map(location=center_loc,
...
def get_locations(car_data):
    locations = car_data.groupby("Start Station ID").first()
...
def get_trip_counts_by_hour(selected_hour, car_data):
    # make a DataFrame with locations for each car station

    locations = get_locations(car_data)
    # select one time of day
    subset = car_data[car_data["hour"] == selected_hour]
...
def create_image_map(image_data, bounds):
    ...
    delta_lat = (max_lon - min_lon) / aspect_ratio * \
        np.cos(min_lat / 360 * 2 * np.pi)

    # add the image to the map
    img = folium.raster_layers.ImageOverlay(map_overlay,
                                            bounds=[
                                                (max_lat - delta_lat, min_lon), (max_lat, max_lon)],
                                            opacity=1,
                                            name="Paths")
